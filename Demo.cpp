...
int main() {
	ariel::Board board;
	board.post(/*row=*/20, /*column=*/20, Direction::Horizontal, "abcd");
	cout << board.read(/*row=*/19, /*column=*/21, Direction::Vertical, /*length=*/3) << endl;  
		// prints "_b_" (starts at row 99 which is empty; then at row 100 there is "b"; then row 101 is empty again).
	board.post(/*row=*/19, /*column=*/22, Direction::Vertical, "xyz");
	cout << board.read(/*row=*/20, /*column=*/20, Direction::Horizontal, /*length=*/6) << endl;  
